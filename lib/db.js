...
exports.getFaucetStats = function(callback) {
	callback = callback || function () {};
	var sql = "SELECT SUM(amount) + (SELECT (SUM(amount) * .25) \
																		FROM transactions \
																		WHERE txid IS NULL \
																		AND referrer IS NOT NULL)  AS outstandingPayments, \
										COUNT(*) AS totalPayments, \
										MAX(ts) AS lastPayment \
										FROM transactions \
										WHERE txid is NULL";
...
exports.getTimeUntilNextDispence = function(address, ip, callback) {
	...
	var sql = "SELECT TIMEDIFF(DATE_ADD(ts, INTERVAL ? MINUTE), NOW()) AS remainingTime, \
				DATE_ADD(ts, INTERVAL ? MINUTE) as nextDispense \
				FROM transactions \
				WHERE (address=? OR ip=?) AND ts > DATE_SUB(NOW(), INTERVAL ? MINUTE)";

	var parms = [settings.payout.frequency, settings.payout.frequency, address, ip, settings.payout.frequency];
...
exports.getPayoutWindow = function(callback) {
	...
	query(sql, parms, function getPayoutWindow_CB(err, rows, fields) {
		return callback(err, firstOrEmpty(rows).payTime);
	});
}
...
exports.logPayments = function(txid, lastPayTime, callback) {
	callback = callback || function () {};
...
exports.logReferralPayments = function(txid, lastPayTime, callback) {
	callback = callback || function () {};

	var sql = "UPDATE transactions \
				SET referral_txid = ? \
				WHERE ts < ? \
				AND referral_txid IS NULL \
				AND referrer IS NOT NULL";
